...
import { DollarSign, TrendingUp, MousePointerClick, Target, Eye, BarChart3 } from "lucide-react"
...
interface CreatorPerformanceTrackerProps {
  creatorId: string
}

export default function CreatorPerformanceTracker({ creatorId }: CreatorPerformanceTrackerProps) {
  ...
  useEffect(() => {
    const fetchPerformance = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/creators/performance?creatorId=${creatorId}&period=${period}`, {
          headers: {
            'X-User-Id': userId || '',
          },
        })
  ...
  const maxImpressions = Math.max(...data.metricsByDate.map(m => m.impressions), 1)
  const maxClicks = Math.max(...data.metricsByDate.map(m => m.clicks), 1)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Track your earnings and conversion data</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.averageEngagementRate.toFixed(2)}%</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.averageConversionRate.toFixed(2)}%</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Daily impressions for the selected period</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <div key={index} className="flex-1 flex flex-col items-center gap-2">
                    <div className="w-full flex flex-col items-center justify-end h-full">
                      <div
                        ...
                        style={{
                          height: `${(metric.impressions / maxImpressions) * 100}%`,
                          minHeight: metric.impressions > 0 ? '4px' : '0',
                        }}
                        title={`${metric.impressions} impressions on ${new Date(metric.date).toLocaleDateString()}`}
                      />
                    </div>
                ...
                <div key={index} className="border rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-1 rounded text-xs ${
                      ...
                      campaign.status === 'accepted' ? 'bg-blue-100 text-blue-800' :
